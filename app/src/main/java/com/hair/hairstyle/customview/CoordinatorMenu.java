...
public class CoordinatorMenu extends FrameLayout {
    ...
    private static final int MENU_CLOSED = 1;   // menu 的状态
    private static final int MENU_OPENED = 2;

    private int mMenuState = MENU_CLOSED;  // menu 默认的状态

    private int mDragOrientation;
    private static final int LEFT_TO_RIGHT = 3;    //从左向右划
    private static final int RIGHT_TO_LEFT = 4;

    private static final float SPRING_BACK_VELOCITY = 1500;

    private static final int SPRING_BACK_DISTANCE = 80;  // 滑动超过这个距离 自动平滑到初始位置或者

    private int mSpringBackDistance;

    private static final int MENU_MARGIN_RIGHT = 64;  // menu 距离右边缘的距离
    ...
    private static final float TOUCH_SLOP_SENSITIVITY = 1.f;  // 滑动的速度
